...
public record SemesterPlan (UniversitySubject[] subjects, SubjectRequirement[] subjectRequirements, int minimalAmoutOfCredits) {
    public SemesterPlan{
        if(subjects == null){
            throw new IllegalArgumentException("Subjects array can not be null!");
        }
        if(subjectRequirements == null){
            throw new IllegalArgumentException("SubjectRequirements array can not be null!");
        }
        if(minimalAmoutOfCredits < 0){
            throw new IllegalArgumentException("MinimalAmountOfCredits can not be negative!");
        }
    }
}
